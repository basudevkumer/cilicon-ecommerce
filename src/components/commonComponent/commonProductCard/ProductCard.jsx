...
const ProductCard = ({ product }) => {
  ...
  return (
    <div className="border border-gray_100 relative p-3 sm:p-4 rounded w-full h-[280px] sm:h-[300px] lg:h-[320px] shadow-xl hover:-translate-y-2 transition duration-500 ease-in-out group overflow-hidden">
      {/* Badge */}
      ...
      <div className="pt-3 sm:pt-4 lg:pt-6">
        <div className="flex items-center gap-x-1">
          ...
          <p className="text-[10px] sm:text-xs text-gray_500">({product.reviews.length})</p>
        </div>
        <p className="text-xs sm:text-sm font-normal text-gray_900 py-1 sm:py-2 truncate">{product.title}</p>
        <div className="flex gap-x-[5px] items-center">
          <del className="text-xs text-gray_400">
            ${(product.price / (1 - product.discountPercentage / 100)).toFixed(2)}
          </del>
          <p className="text-secondary_500 text-xs sm:text-sm font-semibold">${product.price}</p>
        </div>
      </div>
    </div>
  );
