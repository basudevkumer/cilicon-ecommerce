        ...
        <div className="my-8 sm:my-12 lg:my-[72px]">
          {!compareValue?.length ? (
            <div className="text-center py-16">
              ...
              <p className="text-base font-medium text-gray_700">No products to compare</p>
              <p className="text-sm text-gray_500 mt-1">Add products from the shop to compare them.</p>
            </div>
          ) : (
            /* 
            ...
            */
            <div className="overflow-x-auto">
              <div className={`grid gap-0 min-w-[320px]`}
                style={{ gridTemplateColumns: `140px repeat(${compareValue.length}, 1fr)` }}
              >
                {/* Label column */}
                <div className="border border-gray_100">
                  ...
                  {labelRows.map((label, i) => (
                    <div
                      key={i}
                      className={`px-2 sm:px-4 py-2 sm:py-3 ${i % 2 === 0 ? "bg-gray_50" : "bg-gray_00"}`}
                    >
                      <p className="text-gray_700 text-[10px] sm:text-xs font-medium whitespace-nowrap">{label}:</p>
                    </div>
                  ))}
                </div>
